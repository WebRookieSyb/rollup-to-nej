...
const doBuild = async (type, options) => {
  ...
  if(options.include && options.include.length){
    const findInclude = options.include.map(async el => {
      let path = await findFiles(el);
      paths = paths.concat(path)
    });
    await Promise.all(findInclude);
  } else {
    paths = await findFiles(options.input);
  }
...
// 配置输入
const setInputOptions = (type, input, newOutput, alias, removeComments, externalHelpers) => ({
  input: input,
  plugins: setPlugins(type, newOutput, alias, removeComments, externalHelpers),
  onwarn: warning => {
...
function formatOptionsForRollup({
  ...
  const inputOptions = setInputOptions(type, path, newOutput, alias, removeComments, externalHelpers);
