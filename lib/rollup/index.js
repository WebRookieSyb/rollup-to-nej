...
// 配置输入
const setInputOptions = (type, input, newOutput, alias) => ({
  input: input,
  plugins: plugin(type, newOutput, alias),
  onwarn: warning => {
...
function formatOptionsForRollup({
  ...
  const newOutput = path.replace(input, output);
  console.log(`正在打包 '${path}' to '${newOutput}'`);
  // if (path.indexOf(".js") > -1) type = "js";
  console.log(type, path, alias)
  const inputOptions = setInputOptions(type, path, newOutput, alias);
