...
const setPlugins = (type, newOutput, alias, removeComments, externalHelpers) => {
  
  // 使用位运算来配置权限
  // babel的配置默认为vue类型的babel配置
  let target = config.useVue;
  if(externalHelpers) {
    target = config.useVue | config.useExternalHelpers
  }
