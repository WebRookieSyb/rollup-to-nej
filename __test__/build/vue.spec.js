...
const { replaceSrc } = require("../../lib/rollup/rollup-plugin-options");
...
describe("build:vue", () => {
  it("a normal vue component in *.vue", async () => {
    ...
    const realResult = await readFilePromsise(
      "__test__/build/dist/test/test.js",
      "utf-8"
    );
    ...
    ).toEqual(replaceRandomString(realResult));
  });
  it("a jsx vue component in *.vue", async () => {
    ...
    const code = await getCodeFromBundle(bundle, outputOptions);
    const realResult = await readFilePromsise(
      "__test__/build/dist/jsx-test/test.js",
      "utf-8"
    );
